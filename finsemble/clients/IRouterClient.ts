export interface IRouterClient {
	 ...
	 *
	 * @param {string} channel A unique string to identify the channel (must match correspond query channel name); only one responder is allowed per channel.
	 * @param {function} queryEventHandler A function to handle the incoming query (see example below); note the incoming queryMessage contains a function to send the response.
	 * @example
	 *
	 ...
	 *
	 * Only one PubSub responder per topic value in Finsemble application is allowed; however, the topic value may be a regular-expression representing a set of related topics,
	 ...
	 *
	 * See [addPubSubResponder]{@link RouterClientConstructor#addPubSubResponder} for corresponding addition of a PubSub responder.
	 *
