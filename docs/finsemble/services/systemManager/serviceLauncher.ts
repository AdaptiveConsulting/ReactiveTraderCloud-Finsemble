...
export class ServiceLauncher {
	 ...
	 */
	public updateConfig(configs: Object) {
		this.finsembleConfig = configs;
	}
	...
	public getServiceConfig(serviceName): any {
		...
		// For core services (router, config, logging, dataStore) these entries can be added in finsemble-seed/configs/application/manifest-local.json
	...
	private launchServiceAsProcess(worker, workerConfig) {
		...
		const app: any = new System.Application(workerConfig, () => {
			app.run();
