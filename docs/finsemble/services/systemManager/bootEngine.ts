...
export class BootEngine {
	...
	private getBootConfigElement(currentStage: BootStage, name: string, type: BootDependencyType, originalConfig): BootConfigElement {
		...
		// if the name is specified within the config then use that name, just in case it is different from the property name
		newConfigElement.name = originalConfig.name || name;
		...
		if (type === "services") {
			...
			// if the service has debugServiceDelay set, then delay the timeout value an equivalent amount so developer can set breakpoint in service)
			if (configClone.debugServiceDelay && newConfigElement.stage === currentStage) {
				newConfigElement.timeout = newConfigElement.timeout + configClone.debugServiceDelay;
				SystemLog.warn({ leadingBlankLine: false },`==> timeout value for ${name} will be increased to ${newConfigElement.timeout} (debugServiceDelay=${configClone.debugServiceDelay})`);
			}
	...
	private handleOneDependencyStateChange(dependencyName: string, type: BootDependencyType, state: BootDependencyState): void {
		...
		} else if (state === "completed" && type === "bootTasks") {
			// the status for services is published in base service and for components in FSBL, but since tasks are local to system manager will publish status here;
			// publish task status allow support for waitForStartup for tasks
			SystemManagerClient.publishBootStatus(dependencyName, "bootTasks", "completed");
		}
