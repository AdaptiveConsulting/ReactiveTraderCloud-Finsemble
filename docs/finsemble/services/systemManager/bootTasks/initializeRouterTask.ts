...
export class InitializeRouterTask {
	...
	public start(doneCallback: BootTaskCallbackInterface) {
		...
		var isElectron = System.container == "Electron";
		var sameDomainTransport = ConfigUtil.getDefault(finConfig, "finConfig.router.sameDomainTransport", "SharedWorker");
		var crossDomainTransport = ConfigUtil.getDefault(finConfig, "finConfig.router.crossDomainTransport", "IPCBus");
		...
		if (shouldLaunchRouterServer) {
			...
			const IACServerAddress = ConfigUtil.getDefault(finConfig, "finConfig.IAC.serverAddress", defaultServerAddress);
			const ftServerAddress = ConfigUtil.getDefault(finConfig, "finConfig.router.transportSettings.FinsembleTransport.serverAddress", IACServerAddress);
			const ftServerPort = ftServerAddress.substring(ftServerAddress.lastIndexOf(":") + 1); // extract port from address
