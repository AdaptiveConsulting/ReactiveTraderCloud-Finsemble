...
const DistributedStoreClient = require("../../../clients/distributedStoreClient");
const SplinterAgentPool = require("../Splintering/SplinterAgentPool");
const MyWrapManager = require("./WrapManager.ts").WrapManager;
...
class WindowCreation {
	...
	definePubicInterface_Window() {
		...
		RouterClient.addResponder(this.windowServiceChannelName("createWindow"), this.createWindow);
	...
	async createWindow(queryError, queryMessage) {
		var wrapState: WrapState;

		Logger.system.debug(`WindowService-Request.createWindow for ${queryMessage.data.windowDescriptor.name}`, queryMessage.data);

		let { windowDescriptor } = queryMessage.data;
		if (!windowDescriptor) {
		...
		if (err) {
			...
			return queryMessage.sendQueryResponse(err, null);
		}

		let realWindowIdentifier = {
		...
		queryMessage.sendQueryResponse(null, realWindowIdentifier);
	...
	async getWindowIdentifier(queryError, queryMessage) {
		...
		let { err, data } = await MyWrapManager.get(queryMessage.data);
		//data.name = data.name || data.windowName;
		//data.windowName = data.windowName || data.name;
	...
	async injectTitleBar(queryError, queryMessage) {
		...
		this.doTitleBarInjection(queryMessage.data, function (err, data) {
			queryMessage.sendQueryResponse(err, data);
		});
	}
	...
	createStores() {
		const promiseResolver = async (resolve) => {
			Logger.system.debug("LauncherStartup: createGroupStore");
		...
		const promiseResolver = async (resolve) => {
			...
			switch (windowType) {
				...
				case "OpenFinApplication":
					...
					break;
				case "FinsembleNativeWindow": //eslint-disable-line
					var { data } = await this.spawnNativeWindow(windowDescriptor);
					break;
				case "StackedWindow":
	...
	spawnNativeWindow(windowDescriptor): Promise<{ err: any, data: any }> {
		const promiseResolver = (resolve) => {
			...
			RouterClient.addListener(windowDescriptor.name + ".onSpawned", spawnedListener);
			RouterClient.query("FinsembleNative.spawn", windowDescriptor, Function.prototype);
		};
		return new Promise(promiseResolver);
	...
	spawnStackedWindow(componentConfig, cb = Function.prototype): Promise<{ err: any, data: any }> {
		const promiseResolver = (resolve) => {
			...
			Logger.system.debug("Launcher.spawnStackedWindow", componentConfig);
	...
	splinter(windowDescriptor): Promise<{ err: any, data: any }> {
		const promiseResolver = async (resolve) => {
			...
			pool.routeSpawnRequest(windowDescriptor); // sometimes fails here!!!!!!!!!!!!!!!
	...
	createSplinterAgentPool() {
		const promiseResolver = async (resolve) => {
			...
			Logger.system.debug("LauncherStartup: createSplinterAgentPool", finsembleConfig, this.ALLOW_SPLINTERING);
			if (this.ALLOW_SPLINTERING) {
