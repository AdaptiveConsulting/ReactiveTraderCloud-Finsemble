...
/**
 * Tag that inserts the Technical Insights plug-in above a chart.
 *
 * @example <cq-technicalinsights uid="uid" lang="en"></cq-technicalinsights>
 *
...
class TechnicalInsights extends CIQ.UI.ContextTag {
	...
	setContext(context) {
		...
		this.channelSubscribe(
			context.config.channels.technicalinsights,
			(isActive) => {
				if (isActive) {
	...
	notifyChannel(pixelChange) {
		...
		this.channelMergeObject(this.context.config.channels.pluginPanelHeight, {
