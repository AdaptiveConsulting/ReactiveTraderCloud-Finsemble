 ...
 * @param {string} [params.menuSelector] The CSS selector used to identify menu items for
...
CIQ.TimeSpanEventPanel = function (params) {
	...
	this.checkSetPanelHeight = () => {
		...
		if (!(this.panel && stx.panels[TIME_SPAN_EVENT_PANEL])) return;
...
if (CIQ.UI && CIQ.UI.Layout) {
	...
	CIQ.UI.TimeSpanEvent = function (context, params) {
		...
		if (CIQ.TimeSpanEventSample) {
			...
			CIQ.loadScript(
				basePath + "examples/timeSpanEventSample.js",
				function () {
					self.implementation = new CIQ.TimeSpanEventSample(context.stx);
				},
				true
