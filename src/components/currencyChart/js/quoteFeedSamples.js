...
;(function(definition) {
	...
	if (typeof exports === 'object' && typeof module === 'object') {
		...
		throw new Error(
			'Only CommonJS, RequireJS, and <script> tags supported for quoteFeedSamples.js.',
		)
	}
})(function(_exports) {
	...
	CIQ.QuoteFeed.Demo.prototype.update = function(params, cb) {
		...
		if (!current[field]) {
			if (previous[field]) current = previous
			// get series which might be lagging behind a bar
			else field = 'Close'
		}
	...
	CIQ.QuoteFeed.Demo.prototype.generateDaily = function(params, cb) {
		...
		CIQ.postAjax(url, null, function(status, response) {
			...
			cb({
				quotes: newQuotes,
				moreAvailable: false,
				attribution: { source: 'demo', exchange: 'RANDOM' },
	...
	CIQ.QuoteFeed.ChartIQEOD.prototype.fetch = function(params, cb) {
		...
		var self = this
		CIQ.postAjax(url + '/' + symbol.replace(/\//g, '-').toUpperCase(), null, function(
			status,
			response,
		) {
			if (status != 200) {
			...
			if (
				...
				now - CIQ.strToDate(result.quotes[result.quotes.length - 1].Date).getTime() >
					24 * 60 * 60 * 1000
			) {
