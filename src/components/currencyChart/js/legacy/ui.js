...
;(function(definition) {
	...
	CIQ.getPos = function(el) {
		for (var lx = 0, ly = 0; el; lx += el.offsetLeft, ly += el.offsetTop, el = el.offsetParent);
		return { x: lx, y: ly }
	}
			...
			return function(e) {
				...
				CIQ.DropDownManager.callback = (function(dropDown) {
					return function(e) {
						...
						if (
							!CIQ.withinElement(dropDown, e.pageX, e.pageY) &&
							!CIQ.withinElement(menu, e.pageX, e.pageY)
						) {
	...
	CIQ.MenuManager.makeMenus = function() {
		...
		function activate(menuOutline) {
			...
			try {
				...
				eval(action) /*jshint +W061 */ /*eslint no-eval: 1 */
				/*jslint evil: false */
			}
		}
	...
	CIQ.ThemeManager.populateDialog = function(id, stx) {
		...
		function toggleBorders(noDraw) {
			if ($$$('#candleBordersOn', $$(id)).checked) {
				...
				stx.styles.stx_candle_down['border-left-color'] = $$(id).querySelectorAll(
		...
		function toggleMountainGradient() {
			if (mountainGradientCheckbox && mountainGradientCheckbox.checked) {
				stx.styles.stx_mountain_chart.color = CIQ.hexToRgba(
					$$(id).querySelectorAll('.stx-border-color.stx_mountain')[0].style.backgroundColor,
					1,
				)
	...
	CIQ.ThemeManager.createTheme = function(stx) {
		...
		for (var className in CIQ.ThemeManager.classMapping) {
			...
			if (mapping) {
				...
				if (stx.chart.container) theme[className] = stx.chart.container.style[className]
			}
		}
		return theme
	}
	...
	CIQ.DrawingToolbar.prototype.initialize = function(htmlElement) {
		...
		else lineColorPicker.style.backgroundColor = this.stx.currentVectorParameters.currentColor
		...
		else fillColorPicker.style.backgroundColor = this.stx.currentVectorParameters.fillColor
		...
		if (this.stx.currentVectorParameters.annotation.font.size)
			CIQ.DrawingToolbar.setFont(htmlElement, this.stx.currentVectorParameters.annotation.font.size)
	...
	*/
	...
	CIQ.LookupWidget.prototype.displayResults = function(results) {
		...
		for (var i = 0; i < results.length; i++) {
			...
			if (result.description === null || typeof result.description == 'undefined')
			...
			if (this.config.allowSymbolObject) CIQ.ScrollManager.attach(li, select(this, result))
		...
		if (!this.div) {
			...
			if (this.config.filters) {
				...
				for (var i = 0; i < this.config.filters.length; i++) {
					...
					div.appendChild(
						this.config.stx ? CIQ.translatableTextNode(this.config.stx, filter) : filter,
					)
	...
	CIQ.TimeZoneWidget.populateDialog = function(id) {
		...
		if (!CIQ.TimeZoneWidget.iscroll) {
			CIQ.TimeZoneWidget.iscroll = CIQ.iscroll.newScroller('#timezoneDialogWrapper', {
				tap: true,
				scrollbars: false,
				interactiveScrollbars: false,
				mouseWheel: true,
	...
	CIQ.Watch.initialize = function(stxStorageManager, stxLookupWidget) {
		...
		CIQ.Watch.symbolScroll = CIQ.iscroll.newScroller($$$('#stxWatch-symbols').parentNode, {
			tap: true,
			scrollbars: false,
			interactiveScrollbars: false,
			mouseWheel: true,
		})
		CIQ.Watch.listScroll = CIQ.iscroll.newScroller($$$('#stxWatchLists').parentNode, {
			tap: true,
			scrollbars: false,
			interactiveScrollbars: false,
			mouseWheel: true,
	...
	CIQ.Views.initialize = function(stx, stxStorageManager, managePeriodicity) {
		...
		CIQ.Views.scroll = CIQ.iscroll.newScroller($$$('#stxSavedViews').parentNode, {
			tap: true,
			scrollbars: false,
			interactiveScrollbars: false,
			mouseWheel: true,
	...
	CIQ.Studies.studyDialog = function(stx, study, div, override) {
		div.study = study
		div.stx = stx
		if (override && override.inputs && override.inputs.id) div.replaceID = override.inputs.id
	...
	CIQ.Marker.AboveCandle.placementFunction = function(params) {
		...
		for (var i = 0; i < params.arr.length; i++) {
			...
			if (marker.params.xPositioner == 'bar') {
				...
				node.style.left =
					Math.round(stx.pixelFromBar(marker.params.x, chart) - marker.clientWidth / 2) + 1 + 'px'
			} else {
				...
				marker.leftpx = Math.round(
					stx.pixelFromTick(marker.tick, chart) - chart.left - marker.clientWidth / 2,
				)
			...
			if (useHighs) {
				bottom =
					Math.round(
						height - stx.pixelFromPriceTransform(quote.High, panel, yAxis) + bottomAdjust,
					) + 'px'
			} else {
				bottom =
					Math.round(
						height - stx.pixelFromPriceTransform(quote.Close, panel, yAxis) + bottomAdjust,
					) + 'px'
			}
