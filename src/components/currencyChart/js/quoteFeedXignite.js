...
;(function(definition) {
	...
	if (typeof exports === 'object' && typeof module === 'object') {
		...
		throw new Error(
			'Only CommonJS, RequireJS, and <script> tags supported for quoteFeedXignite.js.',
		)
	}
})(function(_exports) {
	...
	CIQ.QuoteFeed.Xignite.prototype.fetch = function(params, cb) {
		...
		var isIndex =
			this.isIndex(symbol) ||
			_checkAssetClass(params.symbolObject.assetClass, CIQ.QuoteFeed.Xignite.Constants.INDEX)
		...
		var isFutureSpread = _checkAssetClass(
			params.symbolObject.assetClass,
			CIQ.QuoteFeed.Xignite.Constants.FUTURE_SPREAD,
		)
		...
		if (!params.symbol) {
				...
				if (isDaily) startDate.setDate(startDate.getDate() - myMaxRecords)
					...
					startDate.setMinutes(
						startDate.getMinutes() -
							myMaxRecords * (isNaN(params.interval) ? params.period : params.interval),
					)
			} else if (params.startDate) {
					...
					if (isDaily) {
						...
						if (params.startDate) {
							...
							args.StartTime =
								...
								' ' +
								CIQ.friendlyDate(pStartDate1).split(' ')[1] +
								':00'
						}
					} else {
						args.StartTime =
							CIQ.mmddyyyy(CIQ.yyyymmdd(startDate)) +
							' ' +
							CIQ.friendlyDate(startDate).split(' ')[1] +
							':00'
						args.EndTime =
							CIQ.mmddyyyy(CIQ.yyyymmdd(myDate)) +
							' ' +
							CIQ.friendlyDate(myDate).split(' ')[1] +
							':59'
					}
				}
			}
				...
				if (params.startDate) {
					...
					if (myDate.getDate() == pStartDate2.getDate()) {
						args.StartTime =
							CIQ.mmddyyyy(CIQ.yyyymmdd(myDate)) +
							' ' +
							CIQ.friendlyDate(pStartDate2).split(' ')[1] +
							':00'
					}
				}
			}
			...
			myDate = toMarketTime(myDate, marketZone)
			if (!params.update && isDaily) {
				if (isIndex) api = CIQ.clone(CIQ.QuoteFeed.Xignite.Templates.HistoricalIndex)
				else api = CIQ.clone(CIQ.QuoteFeed.Xignite.Templates.HistoricalEquity)
				args = {
					Identifier: symbol.replace(/-/g, '/'),
					StartDate: params.startDate
						? CIQ.mmddyyyy(CIQ.yyyymmdd(toMarketTime(params.startDate, marketZone)))
						: null,
					EndDate: CIQ.mmddyyyy(CIQ.yyyymmdd(myDate)),
				...
				if (isDaily) {
					...
					if (!params.startDate && params.maxRecords) {
						...
						startDate.setMinutes(
							startDate.getMinutes() -
								myMaxRecords * (params.interval == 'hour' ? params.period * 60 : params.period),
						)
					} else if (params.startDate) {
						startDate = toMarketTime(params.startDate, marketZone)
					}
					...
					if (params.update) {
						...
						args.StartTime =
							args.StartTime.split(' ')[0] + ' ' + CIQ.friendlyDate(startDate).split(' ')[1] + ':00'
						args.EndTime =
							args.EndTime.split(' ')[0] + ' ' + CIQ.friendlyDate(myDate).split(' ')[1] + ':59'
		...
		if (api && args && error === '') {
					...
					for (i = 1; i < arr.length; i++) {
						...
						if (!isDaily) {
							...
							if (isForex || isFuture) {
								...
								//TODO: get proper exchange times and use timezone lib to convert to EST
								hours = { beginHour: 0, beginMinute: 0, endHour: 23, endMinute: 59 }
						...
						if (cb) {
							if (results.length === 0 && !params.loadMore) {
								...
								cb({
									...
									attribution: { source: 'xignite', exchange: attrExch, charge: charge },
								})
							}
						}
			...
			CIQ.postAjax(
				CIQ.QuoteFeed.Xignite.Utility.url(api, args, params, this.token, identifierType),
				null,
				function(status, res) {
					if (getSplitInfo) {
						...
						CIQ.postAjax(
							CIQ.QuoteFeed.Xignite.Utility.url(
								splitApi,
								splitArgs,
								null,
								this.token,
	...
	CIQ.QuoteFeed.Xignite.Templates = {
		...
		HistoricalEquity: {
			...
			fields:
				'GlobalQuotes.Date,GlobalQuotes.Last,GlobalQuotes.Open,GlobalQuotes.High,GlobalQuotes.Low,GlobalQuotes.Volume,GlobalQuotes.SplitRatio',
			results: {
		...
		HistoricalOptions: {
			...
			fields:
				'ChartBars.StartDate,ChartBars.Open,ChartBars.Close,ChartBars.High,ChartBars.Low,ChartBars.Volume',
			results: {
		...
		IntradayEquity: {
			...
			fields:
				'ChartBars.StartDate,ChartBars.StartTime,ChartBars.UTCOffset,ChartBars.Close,ChartBars.Open,ChartBars.High,ChartBars.Low,ChartBars.Volume,ChartBars.AdjustmentRatio',
			results: {
				date: 'ChartBars StartDate',
		...
		SuperIntradayEquity: {
			...
			statics: 'Precision=Minutes&AdjustmentMethod=None',
			fields:
				'ChartBars.StartDateTime,ChartBars.UTCOffset,ChartBars.Close,ChartBars.Open,ChartBars.High,ChartBars.Low,ChartBars.Volume,ChartBars.AdjustmentRatio',
			results: {
		...
		IntradayBATSRTEquity: {
			...
			statics: 'Precision=Minutes&AdjustmentMethod=None',
			fields:
				'ChartBars.StartDate,ChartBars.StartTime,ChartBars.UTCOffset,ChartBars.Close,ChartBars.Open,ChartBars.High,ChartBars.Low,ChartBars.Volume,ChartBars.AdjustmentRatio,Security.MarketIdentificationCode',
			results: {
				date: 'ChartBars StartDate',
				time: 'ChartBars StartTime',
				open: 'ChartBars Open',
		...
		IntradayIndex: {
			...
			statics: 'Precision=Minutes&AdjustmentMethod=None',
			fields:
				'ChartBars.StartDate,ChartBars.StartTime,ChartBars.UTCOffset,ChartBars.Close,ChartBars.Open,ChartBars.High,ChartBars.Low,ChartBars.Volume',
			results: {
				date: 'ChartBars StartDate',
				time: 'ChartBars StartTime',
				open: 'ChartBars Open',
		...
		IntradayRTForex: {
			...
			fields:
				'ChartBars.StartDate,ChartBars.StartTime,ChartBars.UTCOffset,ChartBars.Open,ChartBars.High,ChartBars.Low,ChartBars.Close,ChartBars.Volume',
			results: {
				date: 'ChartBars StartDate',
				time: 'ChartBars StartTime',
				open: 'ChartBars Open',
