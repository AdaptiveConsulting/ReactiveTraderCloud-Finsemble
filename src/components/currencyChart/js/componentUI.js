...
;(function(definition) {
	...
	if (typeof exports === 'object' && typeof module === 'object') {
		module.exports = definition(
			...
			require('./thirdparty/webcomponents-lite.min'),
		)
	} else if (typeof define === 'function' && define.amd) {
		define([
			'chartiq',
			'thirdparty/object-observe',
			'thirdparty/webcomponents-lite.min',
		], definition)
	...
	CIQ.Marker.HeadsUp.placementFunction = function(params) {
		...
		for (var i = 0; i < params.arr.length; ++i) {
			...
			if (top < 0) {
				...
				bottom =
					(useHighs ? getBottomPixel(stx, panel, containerHeight, quote.Low) : bottom) -
					marker.clientHeight
			} else {
	...
	CIQ.UI.Layout.prototype.showAggregationEdit = function(node, aggregationType) {
		...
		if (!this.aggregationDialog) {
			console.log(
				'Error:  Layout.aggregationDialog must reference your dialog for editing aggregation parameters',
			)
			return
		}
	...
	CIQ.UI.Layout.prototype.showPeriodicity = function(stx, params) {
		...
		if (stx.layout.interval == 'day') {
			...
			stx.layout.interval * stx.layout.periodicity >= 60 &&
	...
	CIQ.UI.MultiWindow.prototype._mru = function(id, create) {
		...
		while (this.stack.length >= this.params.maxWindows) this.stack.pop()
		return this.stack[0]
	}
	...
	CIQ.UI.Prototypes.Dialog.show = function(params) {
		...
		setTimeout(function() {
			...
			if (self.uiManager.overlay && !params.bypassOverlay)
				self.uiManager.overlay.addClass('stx-active')
	...
	CIQ.UI.Prototypes.Themes.persist = function(which) {
		...
		if (!which || which == 'current')
			this.params.nameValueStore.set('CIQ.Themes.current', { theme: this.currentTheme })
		if (!which || which == 'custom')
			this.params.nameValueStore.set('CIQ.Themes.custom', this.params.customThemes)
	}
	...
	CIQ.UI.Prototypes.Lookup.keyStroke = function(hub, key, e, keystroke) {
		...
		if (result) {
			setTimeout(function() {
				...
				if (
					(input.value.length || key == 'backspace' || key == 'delete') &&
		...
		if (oldVal === null && typeof newVal == 'string') {
			if (this.context.events) {
				...
				if (this.context.events.symbolChange)
					this.context.stx.removeEventListener(this.context.events.symbolChange)
			}
	...
	CIQ.UI.Prototypes.TradingCentral.initialize = function() {
		...
		var done0 = function(err) {
			...
			if (++count0 === 3) {
				self.context.tc = new CIQ.TradingCentral(
					self.context.stx,
	...
	CIQ.UI.Prototypes.TradingCentral.lineHoverListen = function() {
		...
		var lineInfo = document
			.getElementById('tc-line-info-tmpl')
			.content.querySelector('tc-line-info')
		var stxx = this.context.stx
		...
		this.addEventListener('linehoverbegin', function(hover) {
			...
			newNode.appendChild(
				document.createTextNode(parts.description + parts.trend[hover.detail.trend]),
			)
	...
	CIQ.UI.Prototypes.TradingCentral.run = function() {
		...
		this.context.events.layout = tc.stx.addEventListener('layout', function() {
			...
			node.appendChild(
				document.createTextNode('Go back to ' + dom.termButtons[currentTerm].textContent),
			)
	...
	CIQ.UI.Prototypes.ChartTitle.update = function() {
		...
		if (symbolDescriptionDiv.length) {
			symbolDescriptionDiv.text(
				stx.chart.symbolDisplay ? stx.chart.symbolDisplay : stx.chart.symbol,
			)
		}
		...
		if (stx.chart.symbol) {
			this.title = stx.chart.symbol + ' \u200b \u200b ' + currentPrice + ' \u200b \u200b \u200b '
			...
			outer: for (var study in this.layout.studies) {
				...
				if (self.messages.sources[type]) {
					for (var i = 0; i < this.markers.attribution.length; i++) {
						if (this.markers.attribution[i].params.panelName == this.layout.studies[study].panel)
							continue outer
					}
	...
	CIQ.UI.UIManager = document.registerElement('cq-ui-manager', {
		prototype: CIQ.UI.Prototypes.UIManager,
	})
	 ...
	 */
	CIQ.UI.Advertisement = document.registerElement('cq-advertisement', {
		prototype: CIQ.UI.Prototypes.Advertisement,
	})
	...
	CIQ.UI.MenuDropDown = document.registerElement('cq-menu-dropdown', {
		prototype: CIQ.UI.Prototypes.MenuDropDown,
	})
	 ...
	 * @constructor
	 */
	CIQ.UI.DrawingToolbar = document.registerElement('cq-toolbar', {
		prototype: CIQ.UI.Prototypes.DrawingToolbar,
	})

	CIQ.UI.ShowRange = document.registerElement('cq-show-range', {
		prototype: CIQ.UI.Prototypes.ShowRange,
	})

	CIQ.UI.Loader = document.registerElement('cq-loader', { prototype: CIQ.UI.Prototypes.Loader })

	CIQ.UI.ColorPicker = document.registerElement('cq-color-picker', {
		prototype: CIQ.UI.Prototypes.ColorPicker,
	})
	 ...
	 */
	CIQ.UI.SidePanel = document.registerElement('cq-side-panel', {
		prototype: CIQ.UI.Prototypes.SidePanel,
	})
	CIQ.UI.ChartTitle = document.registerElement('cq-chart-title', {
		prototype: CIQ.UI.Prototypes.ChartTitle,
	})

	CIQ.UI.Attribution = document.registerElement('cq-attribution', {
		prototype: CIQ.UI.Prototypes.Attribution,
	})
	CIQ.UI.ViewDialog = document.registerElement('cq-view-dialog', {
		prototype: CIQ.UI.Prototypes.ViewDialog,
	})
	...
	CIQ.UI.Comparison = document.registerElement('cq-comparison', {
		prototype: CIQ.UI.Prototypes.Comparison,
	})
	CIQ.UI.SymbolLookup = document.registerElement('cq-lookup', {
		prototype: CIQ.UI.Prototypes.Lookup,
	})

	CIQ.UI.StudyDialog = document.registerElement('cq-study-dialog', {
		prototype: CIQ.UI.Prototypes.StudyDialog,
	})
	CIQ.UI.StudyOutput = document.registerElement('cq-study-output', {
		prototype: CIQ.UI.Prototypes.StudyOutput,
	})
	CIQ.UI.StudyInput = document.registerElement('cq-study-input', {
		prototype: CIQ.UI.Prototypes.StudyInput,
	})
	CIQ.UI.StudyParameter = document.registerElement('cq-study-parameter', {
		prototype: CIQ.UI.Prototypes.StudyParameter,
	})
	CIQ.UI.StudyLegend = document.registerElement('cq-study-legend', {
		prototype: CIQ.UI.Prototypes.StudyLegend,
	})

	CIQ.UI.Themes = document.registerElement('cq-themes', { prototype: CIQ.UI.Prototypes.Themes })
	CIQ.UI.ThemeDialog = document.registerElement('cq-theme-dialog', {
		prototype: CIQ.UI.Prototypes.ThemeDialog,
	})
	CIQ.UI.ThemePiece = document.registerElement('cq-theme-piece', {
		prototype: CIQ.UI.Prototypes.ThemePiece,
	})

	CIQ.UI.TFC = document.registerElement('cq-tfc', { prototype: CIQ.UI.Prototypes.TFC })

	CIQ.UI.TradingCentral = document.registerElement('cq-tradingcentral', {
		prototype: CIQ.UI.Prototypes.TradingCentral,
	})

	CIQ.UI.BasicDetails = document.registerElement('cq-details', {
		prototype: CIQ.UI.Prototypes.BasicDetails,
	})

	CIQ.UI.CqTcNumber = document.registerElement('cq-tc-number', {
		prototype: CIQ.UI.Prototypes.CqTcNumber,
	})
