	...
	FSBL.Clients.ConfigClient.getValue({ field: 'finsemble.appDirectoryEndpoint' }, function(
		...
		getToolbarStore((err, response) => {
			FSBL.Clients.RouterClient.subscribe(
				'Finsemble.Service.State.launcherService',
				(err, response) => {
	...
	FSBL.Clients.LauncherClient.getComponentList((err, componentList) => {
		...
		Object.keys(folders).forEach(folderName => {
			...
			appsName.forEach(appName => {
				...
				if (
					!componentNameList.includes(appName) &&
					!apps.includes(folders[folderName]['apps'][appName]['appID'].toString())
				) {
		...
		componentNameList.map(componentName => {
			...
			if (launchableByUser) {
				data.configComponents[componentName] = {
					appID: componentName,
					icon:
						component.foreign.Toolbar && component.foreign.Toolbar.iconClass
							? component.foreign.Toolbar.iconClass
							: null,
					name: componentName,
...
function reorderFolders(destIndex, srcIndex) {
	...
	const unorderableFolders = data.foldersList.filter(folderName =>
...
function deleteApp(appID) {
	ToolbarStore.removeValue(
		{ field: 'pins.' + data.apps[appID].name.replace(/[.]/g, '^DOT^') },
		(err, res) => {
			if (err) {
