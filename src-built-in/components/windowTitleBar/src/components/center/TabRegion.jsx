...
export default class TabRegion extends React.Component {
	...
	startDrag(e, windowIdentifier) {
		...
		e.dataTransfer.setData(
			...
			JSON.stringify(windowIdentifier)
		);
	...
	extractWindowIdentifier(e) {
		try {
			let identifier = JSON.parse(
				e.dataTransfer.getData("application/fsbl-tab-data")
			);
			...
			if (typeof identifier.windowName !== "undefined") {
				...
				FSBL.Clients.Logger.system.error(
					"Malformed drop object detected in windowTitleBar. Check tab droppping code. Expected windowIdentifier, got ",
					identifier
				);
				return null;
			}
	...
	drop(e) {
		...
		if (
			...
			identifier.windowName === finsembleWindow.name
		)
			return FSBL.Clients.WindowClient.cancelTilingOrTabbing();
		FSBL.Clients.WindowClient.stopTilingOrTabbing(
			{ allowDropOnSelf: true, action: "tabbing" },
			() => {
				...
				if (identifier && identifier.windowName) {
					...
					subscribeId = FSBL.Clients.RouterClient.subscribe(
						"Finsemble." + identifier.guid,
						tabIdentifierSubscriber
					);
				} else {
					FSBL.Clients.Logger.system.error(
	...
	isTabRegionOverflowing() {
		...
		return (
			lastTab.right + this.state.translateX >
			this.state.boundingBox.right - this.state.boundingBox.left
		);
	}
	...
	onMouseWheel(e) {
		...
		if (numTabs > 1) {
			...
			if (newLeftForFirstTab < 0) {
				...
				newRightForLastTab <= boundingBox.right
			) {
	...
	getTabClasses(tab) {
		...
		if (
			...
			tab.windowName === this.state.activeTab.windowName
		) {
	...
	onTabAdded(identifier, newIndex) {
		...
		if (
			this.state.tabs.length === 1 &&
			identifier.windowName == myIdentifier.windowName
		) {
			return;
		}
	...
	componentWillMount() {
		...
		Store.addListener(
			...
			this.onTabListTranslateChanged
		);
	}
	...
	render() {
		...
		let tabRegionDropZoneStyle = {
			left: this.state.tabs.length * this.state.tabWidth + "px"
		};
