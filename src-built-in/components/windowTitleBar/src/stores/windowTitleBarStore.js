...
var Actions = {
		...
		if (windowTitleBarConfig.showLinker) {
			...
			let cb = (err, response) => {
				if (response.channels) {
					windowTitleBarStore.setValue({ field: 'Linker.channels', value: response.channels })
				}
					...
					if (opts.customData && opts.customData.foreign && opts.customData.foreign.services) {
						if (opts.customData.foreign.services.windowService) {
							if (opts.customData.foreign.services.windowService.allowTabbing !== undefined) {
								...
								opts.customData.foreign.services.windowService.ignoreTilingAndTabbingRequests !=
								undefined
							) {
							...
							if (opts.customData.foreign.services.dockingService.allowTabbing !== undefined) {
								...
								opts.customData.foreign.services.dockingService.ignoreTilingAndTabbingRequests !=
								undefined
							) {
	...
	removeTabLocally: function(windowIdentifier) {
		...
		let i = tabs.findIndex(
			el => el.name === windowIdentifier.name && el.uuid === windowIdentifier.uuid,
		)
		tabs.splice(i, 1)
	...
	setupStore: function(cb = Function.prototype) {
		...
		constants.TAB_SCROLL_POSITION_CHANGED =
			Actions.parentWrapper.name + '.tabListTabListScrollPositionChanged'
