	...
	initialize: function() {
		...
		if (FSBLHeader) {
			...
			windowTitleBarStore.setValues([
				...
				{
					field: "Minimize.hide",
					value: FSBLHeader.hideMinimize ? true : false
				},
				{ field: "Close.hide", value: FSBLHeader.hideClose ? true : false },
				{
					field: "AlwaysOnTop.show",
					value: FSBLHeader.alwaysOnTop ? true : false
				}
			]);
		 ...
		 */
		FSBL.Clients.WindowClient.finWindow.addEventListener(
			"maximized",
			function() {
				self.clickMaximize();
			}
		...
		FSBL.Clients.ConfigClient.getValue({ field: "finsemble" }, function(
			err,
			finsembleConfig
		) {
				...
				finsembleWindow.getOptions((err, opts) => {
					if (
						...
						if (opts.customData.foreign.services.windowService) {
							...
							if (
								opts.customData.foreign.services.dockingService.allowTabbing !==
								undefined
							) {
								showTabs =
									opts.customData.foreign.services.dockingService.allowTabbing;
