...
var Actions = {
	initialize: function(cb) {
		async.parallel(
			[Actions.initializeComponentList, Actions.getUserDefinedComponentList, setupHotkeys],
			function(err) {
				if (err) {
					console.error(err)
				}
	...
	getUserDefinedComponentList(cb) {
		return cb()
		var self = Actions
		FSBL.Clients.StorageClient.get({ topic: 'finsemble', key: 'userDefinedComponents' }, function(
			err,
			response,
		) {
			var data = response
...
function getWindowGroupStore(done) {
	FSBL.Clients.DistributedStoreClient.getStore(
		{ global: true, store: 'Finsemble-WindowGroups' },
		function(err, store) {
...
function initialize(cb) {
	...
	async.parallel(
		[createLocalStore, getToolbarStore, getWindowGroupStore, Actions.initialize],
		function(err) {
			if (err) {
				console.error(err)
			}
