...
Actions = {
	...
	initialize: function() {
		 ...
		 */
		FSBL.Clients.RouterClient.subscribe('Finsemble.WorkspaceService.update', function(
			err,
			response,
		) {
	...
	reorderWorkspaceList: function(changeEvent) {
		...
		let workspaces = JSON.parse(
			JSON.stringify(WorkspaceManagementStore.getValue({ field: 'WorkspaceList' })),
		)
...
function getToolbarStore(done) {
	...
	async.retry(
		...
		(callback, results) => {
			StoreClient.getStore({ global: true, store: 'Finsemble-Toolbar-Store' }, function(
				err,
				store,
			) {
