...
export default class WorkspaceMenuOpener extends React.Component {
	...
	addListeners() {
		ToolbarStore.Store.addListener(
			...
			this.receiveActiveWorkspaceName,
		)
		ToolbarStore.Store.addListener(
			{ field: 'workspaceMenuWindowName' },
			this.receiveWorkspaceMenuWindowName,
		)
	}
	removeListeners() {
	...
	render() {
		...
		return (
			<FinsembleButton
				menuWindowName={'Workspace Management Menu'}
				preSpawn={true}
				buttonType={['MenuLauncher', 'Toolbar']}
				className={'finsemble-toolbar-workspace-button-label workspace-menu-button'}
				label={this.state.activeWorkspaceName}
