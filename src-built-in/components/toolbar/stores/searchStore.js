...
var Actions = {
			...
			if (window.outerWidth < 400) {
				finsembleWindow.getBounds((err, bounds) => {
					...
					finsembleWindow.animate(
						{ transitions: { size: { duration: 150, width: 400 } } },
						Function.prototype,
					)
				})
			}
	...
	positionSearchResults() {
		...
		if (!bounds || !bounds.left) {
			FSBL.Clients.Logger.error(
				'No bounds received from inputContainerBoundsHandler.  Assuming {left: 0}.',
			)
			bounds = { left: 0 }
		}
	...
	handleClose(e) {
		...
		menuWindow.isShowing(function(err, showing) {
			...
			if (showing) {
				...
				if (!e && cachedBounds) {
					finsembleWindow.animate(
						{ transitions: { size: { duration: 150, width: cachedBounds.width } } },
						() => {
							cachedBounds = null
						},
					)
	...
	setupWindow(cb = Function.prototype) {
		...
		FSBL.FinsembleWindow.getInstance({ windowName: menuReference.name }, (err, wrap) => {
			if (err) {
				FSBL.Clients.Logger.error(
					`Failed to retrieve reference to search results menu: ${menuReference.name}, error:`,
					err,
				)
			}
			menuWindow = wrap
...
function createStore(done) {
	...
	FSBL.Clients.DistributedStoreClient.createStore(
		{ store: 'Finsemble-SearchStore-' + finsembleWindow.name, values: defaultData, global: true },
		function(err, store) {
			if (err) {
				FSBL.Clients.Logger.error(
					`DistributedStoreClient.createStore failed for store Finsemble-SearchStore-${finsembleWindow.name}, error:`,
					err,
				)
			}
			menuStore = store
			...
			store.getValues(['owner', 'menuSpawned'], function(err, data) {
				if (err) {
					FSBL.Clients.Logger.error(
						`DistributedStoreClient.createStore->store.getValues, error:`,
						err,
					)
				}
				...
				if (!data.menuSpawned) {
					FSBL.Clients.LauncherClient.spawn(
						'searchMenu',
						{ name: 'searchMenu.' + finsembleWindow.name, data: { owner: finsembleWindow.name } },
						function(err, data) {
							if (err) {
					...
					menuStore.getValue('menuIdentifier', function(err, menuIdentifier) {
						if (err) {
							FSBL.Clients.Logger.error(
								`DistributedStoreClient.createStore->menuStore.getValue, error:`,
								err,
							)
						}
...
function initialize(cb) {
	...
	async.parallel([createStore], function(err) {
		if (err) {
			console.error(err)
		}
